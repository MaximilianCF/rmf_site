...
pub fn make_dagger_mesh() -> Mesh {
    ...
    make_cuboidy_wrap([lower_ring, upper_ring], segments)
        .merge_into(&mut mesh)
        .expect("merge dagger body");
    make_pyramid(upper_ring, [0., 0., top_height], segments)
        .merge_into(&mut mesh)
        .expect("merge dagger tip");
    make_pyramid(lower_ring.flip_height(), [0., 0., 0.], segments)
        .transform_by(Affine3A::from_quat(Quat::from_rotation_y(
            180_f32.to_radians(),
        )))
        .merge_into(&mut mesh)
        .expect("merge dagger base");
    return mesh;
}
...
pub fn make_cylinder_arrow_mesh() -> Mesh {
    ...
    make_cone(head_base, tip, resolution)
        .merge_into(&mut mesh)
        .expect("merge cylinder arrow head");
    make_smooth_wrap([cylinder_top, cylinder_bottom], resolution)
        .merge_into(&mut mesh)
        .expect("merge cylinder arrow body");
    make_smooth_wrap([head_base, cylinder_top], resolution)
        .merge_into(&mut mesh)
        .expect("merge cylinder arrow neck");
    make_bottom_circle(cylinder_bottom, resolution)
        .merge_into(&mut mesh)
        .expect("merge cylinder arrow base");
    return mesh;
}
